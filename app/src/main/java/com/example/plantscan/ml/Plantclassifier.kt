...
class PlantClassifier(private val context: Context) {
    ...
    companion object {
        ...
        // ── Public disease database (accessible by EncyclopediaScreen) ──────────
        val DISEASE_INFO: Map<String, Map<String, String>> = mapOf(
    ...
    init {
        interpreter  = Interpreter(loadModelFile(), Interpreter.Options().apply { setNumThreads(4) })
    ...
    fun classify(bitmap: Bitmap): List<PredictionResult> {
        ...
        return output[0]
            ...
            .map { (index, confidence) ->
                ...
                val info      = DISEASE_INFO[className]
                PredictionResult(
                    className   = className,
                    ...
                    symptom     = info?.get("ciri-ciri") ?: "-",   // ← fix: was "gejala"
